...
\begin{enumerate}
    ...
    \begin{gather*}
    \overline{ac}>\overline{ab} \implies \exists p \in \overline{ac} \text{ tal que } \overline{ap}\equiv \overline{ab} \therefore \triangle abp \text{ es isósceles. Luego,}\\
    \angle abc > \angle abp \text{ (por definición), } \angle abp \equiv \angle apb \text{ (teorema 34) y } \angle apb> \angle acb \text{ (Teorema 39). De allí resulta}\\
    \angle abc>\angle acb.
    \end{gather*}
    \begin{figure}[H]
        \centering
        \includegraphics[scale=0.5]{imagenes/T41.png}
    \end{figure}
    \item Supongamos ahora $\angle abc > \angle acb$, queremos ver $\overline{ac}>\overline{ab}$. Comparando $\overline{ac}$ con $\overline{ab}$, hay 3 posibilidades:
    \begin{itemize}
        \item $\overline{ab}<\overline{ac}$
        \item $\overline{ab} \equiv \overline{ac} \implies \angle acb \equiv \angle abc$ (Teorema 34). Absurdo.
        \item $\overline{ab}>\overline{ac} \implies \angle acb> \angle abc$, absurdo.
    \end{itemize}
    Luego debe ser $\overline{ab}<\overline{ac}$
\end{enumerate}
\subsection{\textcolor{blue}{Teorema 44}}
\subsubsection{Enunciado}
