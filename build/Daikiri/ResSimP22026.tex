...
\begin{enumerate}
    ...
    \begin{enumerate}
        ...
        \end{itemize}
        \item Los valores que cumplen tal condición son los que respectan a la parte senoidal y cuadrática, entonces $\{x \in \mathbb{R}: f(x)\leq 3\}=[-4,-1)\cup(1,7]$
        \item Sabemos que $f(x)$ corresponde a la imagen de la función, para encontrar el valor que le corresponde al punto dado se puede imaginar una recta horizontal que corta el eje de las ordenadas en $4$ y observar dónde corta a la función dada (aunque puede no haber tales intersecciones), los valores de $x$ corresponden a los valores de las abscisas de los puntos de intersección entre la recta y el gráfico de la función. Para este ejercicio el valor de $x$ que cumple $f(x)=4$ es $x=1$. Ahora para determinar el valor de la imagen que me devuelven evaluar $f(x)$ en $-3$ y $4$ es análogo pero con una linea vertical que corta el eje de las abscisas y observando dónde corta nuevamente a $f(x)$ (si es que sucede), en este ejercicio la imagen de $f(-3)$ es $0$ y de $f(4)$ es $-4$.
